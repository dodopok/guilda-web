...
const { data, refresh } = await useAsyncData(() => `template-${id.value}`, async () => {
  const templateId = id.value
  const [t, c, list] = await Promise.all([
    capi<{ template: TemplateFull }>(`/templates/${templateId}`),
    capi<{ duties: Duty[] }>('/catalog'),
    capi<{ templates: { id: string, name: string, kind: string, archived: boolean, blockCount: number }[] }>('/templates'),
  ])
  return { template: t.template, duties: c.duties.filter((d) => d.active), templates: list.templates.filter((x) => !x.archived) }
})
...
async function doSave(apply: boolean) {
  ...
  saving.value = true
  try {
    const r = await capi<{ updatedScripts: number }>(`/templates/${id.value}`, { method: 'PATCH', body: { name: form.name, kind: form.kind, blocks: toBlocks(items.value), applyToUpcoming: apply } })
...
<template>
  <div
    v-if="data"
    class="template-edit-layout"
  >
    <aside class="template-edit-list">
      <div
      ...
      <nav
        ...
        aria-label="Modelos de liturgia"
      >
        <NuxtLink
          v-for="template in data.templates"
          :key="template.id"
          :to="link(`/coordenacao/modelos/${template.id}`)"
          class="template-edit-list__item"
          :class="{ 'template-edit-list__item--current': template.id === id }"
          :aria-current="template.id === id ? 'page' : undefined"
        >
          <span class="template-edit-list__count">{{ template.blockCount }}</span>
          <span class="grow">
            <span class="template-edit-list__name">{{ template.name }}</span>
            <span class="template-edit-list__sub">{{ TEMPLATE_KIND[template.kind] ?? template.kind }}</span>
          </span>
        </NuxtLink>
        <p
