...
const pendingAvailability = computed(() => data.value?.availability.filter((x) => !x.responded) ?? [])
  ...
  <section class="stack">
    ...
    <template v-if="data">
      <div class="member-home-grid">
        <div class="member-home-primary stack-lg">
          <section v-if="next">
            <p class="section-label">
              Sua próxima escala · {{ relativeDay(next.service.startsAt, tz) }}
            </p>
            <article class="card card--lg card--flush card--rel member-home-next-card">
              <svg
                viewBox="0 0 120 120"
                aria-hidden="true"
                class="rings"
              ...
              /></svg>
              <div
                class="row member-home-next-head"
                style="flex-wrap:nowrap;gap:16px;padding:20px 20px 16px;border-bottom:1px solid var(--line-2)"
              >
                <DateTile
        ...
        <aside class="member-home-side stack-md">
          <AvailabilityPicker
            v-if="pendingAvailability[0]"
            :month="pendingAvailability[0].month"
            embedded
            @submitted="refresh"
          />
          <NuxtLink
            v-for="a in pendingAvailability.slice(1)"
            :key="a.month"
          ...
          >
            <span class="cta-card__icon"><Icon
              ...
              :weight="1.9"
            /></span>
            <span class="grow"><span class="cta-card__title">Responder disponibilidade de {{ monthName(a.month) }}</span><span class="cta-card__sub">Até {{ weekdayLong(a.deadlineAt, tz).replace('-feira', '') }}, {{ dayMonth(a.deadlineAt, tz) }}</span></span>
            <Icon
              name="chevron-right"
              :weight="2"
          ...
          >
            <span class="cta-card__icon"><Icon
              ...
              :weight="1.9"
            /></span>
            <span class="grow"><span class="cta-card__title">{{ data.swapsWaiting === 1 ? 'Davi pede que você assuma uma tarefa' : `${data.swapsWaiting} pedidos para você assumir tarefas` }}</span><span class="cta-card__sub">A troca só vale se você aceitar</span></span>
            <Icon
              name="chevron-right"
              :weight="2"
              class="cta-card__chev"
